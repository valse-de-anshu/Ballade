...
function Timeline:render()
	...
	if is_line then
		...
		-- YouTube style thin line at the bottom
		local bar_h = (size > 2) and 4 or 2
		bay = bby - bar_h
		fay = bay
	else
	...
	local function draw_progress()
		...
		if is_line then
			local radius = (size > 2) and 6 or 0
			if radius > 0 then
				local line_cy = fay + (fby - fay) / 2
				ass:circle(fbx, line_cy, radius, {opacity = config.opacity.position})
			end
		end
	end
